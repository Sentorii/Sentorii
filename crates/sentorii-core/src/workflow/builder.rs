...
impl<R: CommandRunner> WorkflowBuilder<R> {
    ...
    pub async fn build(self) -> Result<Workflow<R>, CoreError> {
        let ui_steps = self.steps.iter().enumerate().map(|(v, i)| Step::ui_step(i, v)).collect();
...
mod tests {
    ...
    fn test_builder_accumulates_steps_correctly() {
        ...
        let builder = WorkflowBuilder::new(workflow_id, "test".to_string(), tx, runner, git_root, context);
    ...
    async fn test_builder_emits_events_correctly_on_build() {
        ...
        let workflow = WorkflowBuilder::new(workflow_id, "test".to_string(), event_tx, runner, git_root, context)
